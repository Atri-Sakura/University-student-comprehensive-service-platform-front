<template>
  <view class="second-hand-orders-container">
    ...
    <view class="order-list" v-if="orders.length > 0">
      <view class="order-item" v-for="order in orders" :key="order.orderMainID">
        <!-- 状态标签 -->
        ...
        <view class="product-info">
          ...
          <view class="product-details">
            <text class="product-name">{{ order.pickAddress || '二手交易订单' }}</text>
    ...
    async getSecondHandOrders() {
      try {
        ...
        if (res && res.code === 200) {
          ...
          this.orders = orderList.filter(item => item.orderType === 3);
        } else {
