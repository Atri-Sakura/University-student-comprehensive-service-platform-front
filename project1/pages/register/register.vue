...
// 定义API基础URL常量，用于直接构建请求URL
const API_BASE_URL = 'http://localhost:8080/platform/auth';
...
export default {
	...
	methods: {
		...
		async refreshCaptcha() {
			...
			// 然后尝试从服务器获取验证码
			try {
				// 注意这里使用的是基础URL，没有包含/platform/auth路径
				const response = await fetch('http://localhost:8080/captchaImage', {
					method: 'GET',
					headers: {
		...
		async handleRegister() {
			...
			try {
				...
				if (result.code === 200 || result.success) {
					...
					// 注册失败
					this.showMessage(result.message || '注册失败，请检查信息', 'none');
					// 刷新验证码
					this.refreshCaptcha();
				}
			} catch (error) {
