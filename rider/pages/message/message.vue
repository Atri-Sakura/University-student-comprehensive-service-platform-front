	...
	<view class="container">
		...
		<view class="message-list">
			...
			<template v-if="activeTab === 'all'">
				...
				<!-- 客服消息 -->
				<view class="message-item service" @tap="openCustomerService">
					<view class="message-avatar">
						<text class="avatar-icon service">👥</text>
						<view class="unread-dot" v-if="hasUnreadService"></view>
					</view>
					<view class="message-content">
						<view class="message-header">
							<text class="message-title">客服消息</text>
							<text class="message-time">{{ lastServiceTime }}</text>
						</view>
						<text class="message-preview" :class="{ unread: hasUnreadService }">{{ lastServiceMessage }}</text>
					</view>
					<text class="message-arrow">〉</text>
				</view>

				<!-- 平台公告 -->
				<view class="message-item announcement" @tap="viewMessage(item)" v-for="item in announcements" :key="item.id">
					<view class="message-avatar">
						<text class="avatar-icon announcement">📢</text>
						<view class="unread-dot" v-if="!item.isRead"></view>
					</view>
					<view class="message-content">
						<view class="message-header">
			...
			<template v-else-if="activeTab === 'group'">
				<!-- 订单群聊消息 -->
				<view class="message-item group" @tap="viewGroupChat(item)" v-for="item in orderGroupChats" :key="item.id">
					<view class="message-avatar">
					...
					<view class="message-content">
						<view class="message-header">
							<text class="message-title">{{ item.orderTitle }}</text>
							<text class="message-time">{{ formatTime(item.time) }}</text>
						</view>
						<view class="order-info">
							<text class="order-id">订单号：{{ item.orderId }}</text>
							<text class="participants">参与者：{{ item.participants.join('、') }}</text>
						</view>
						<text class="message-preview" :class="{ unread: !item.isRead }">{{ item.lastMessage }}</text>
					</view>
			...
			<!-- 空状态 -->
			<view class="empty-state" v-if="currentTabMessages.length === 0">
