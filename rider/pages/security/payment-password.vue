<template>
	<view class="page">
		...
		<view class="content">
			<!-- 当前状态卡片 -->
			<view class="status-card" @tap="handleSetPassword" v-if="!hasPaymentPassword">
				<view class="status-icon">
					<text class="icon">🔒</text>
				</view>
				<view class="status-info">
					<text class="status-title">设置支付密码</text>
					<text class="status-desc">点击设置6位数字支付密码</text>
				</view>
				<text class="card-arrow">〉</text>
			</view>
			
			<view class="status-card active" v-if="hasPaymentPassword">
				<view class="status-icon">
					<text class="icon">✓</text>
				</view>
				<view class="status-info">
					<text class="status-title">支付密码</text>
					<text class="status-desc enabled">已设置</text>
				</view>
			</view>

			<!-- 功能选项 -->
			<view class="options-section" v-if="hasPaymentPassword">
				<view class="option-item" @tap="handleChangePassword">
...
<script>
import { setPayPassword, updatePayPassword } from '@/utils/api/index.js';
...
export default {
	...
	methods: {
		...
		handleChangePassword() {
			// 跳转到验证原支付密码页面
			uni.navigateTo({
				url: '/pages/security/verify-payment-password'
			});
		},
		...
		async submitPassword() {
			try {
				uni.showLoading({
					title: this.modalType === 'set' ? '设置中...' : '修改中...',
					mask: true
				});
				...
				if (this.modalType === 'set') {
					// 设置支付密码
					result = await setPayPassword(this.newPassword, this.confirmPassword);
				} else {
					// 修改支付密码
					result = await updatePayPassword(this.currentPassword, this.newPassword, this.confirmPassword);
				}
				...
				if (result.code === 200) {
					uni.showToast({
						title: result.msg || (this.modalType === 'set' ? '支付密码设置成功' : '支付密码修改成功'),
						icon: 'success',
						duration: 2000
					});
